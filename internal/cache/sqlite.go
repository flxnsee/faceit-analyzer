...
func (c *Cache) GetMatches(playerID string) ([]Match, error) {
	rows, err := c.db.Query(`
		SELECT player_id, match_id, game_mode, map_name, result, elo_change, elo_after, played_at
	...
	for rows.Next() {
		var m Match
		if err := rows.Scan(&m.PlayerID, &m.MatchID, &m.GameMode, &m.MapName, &m.Result, &m.EloChange, &m.EloAfter, &m.PlayedAt); err != nil {
			return nil, err
		}
...
func (c *Cache) SaveMatches(matches []Match) error {
	...
	stmt, err := tx.Prepare(`
		INSERT INTO match_history (player_id, match_id, game_mode, map_name, result, elo_change, elo_after, played_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	...
	for _, m := range matches {
		if _, err := stmt.Exec(m.PlayerID, m.MatchID, m.GameMode, m.MapName, m.Result, m.EloChange, m.EloAfter, m.PlayedAt); err != nil {
			return err
		}
	}
	return tx.Commit()
...
func (c *Cache) GetMatchStats(matchID, playerID string) (*MatchStats, error) {
	...
	err := c.db.QueryRow(`
		SELECT match_id, player_id, kills, deaths, assists, headshots, rounds_played,
		       pistol_rounds_won, pistol_rounds_total,
		       overtime_rounds_won, overtime_rounds_total,
		...
		&s.PistolRoundsWon, &s.PistolRoundsTotal,
